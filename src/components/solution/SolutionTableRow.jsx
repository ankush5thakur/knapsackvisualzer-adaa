...
function SolutionTableRow({
  ...
  function shouldDisplayValue(index, cell) {
    return (
      cell &&
      cell.column === index &&
      cell.type === CellType.contributingCellLeftOverCapacity
    );
  }
      ...
      {row.map((cell, index) => {
        ...
        return (
          <td
            id={id}
            key={id}
            className={`cell ${currentCSS}`}
            ref={rowIndex === 1 && index === 1 ? measureCellRef : null}
          >
            {displayValue ? (
...
SolutionTableRow.propTypes = {
  ...
  item: ItemPropType,
  highlightcells: PropTypes.oneOfType([
    TableStateReturnValue,
    PropTypes.arrayOf(TableStateReturnValue),
  ]),

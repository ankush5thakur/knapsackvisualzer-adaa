...
function SetupScreen({ items, capacity }) {
  ...
  function onSubmit(event) {
    ...
    appContext.appDispatch({
      ...
      capacityValue: value,
      items: event.itemsArray,
    });
  }

  // Function to apply conditional CSS based on errors
  function getCSS(index, fieldName) {
    if (
      errors.itemsArray &&
      errors.itemsArray[index] &&
      errors.itemsArray[index][fieldName]
    ) {
      return (
        "border-1 border-red-500 focus:border-2 focus:border-red-500 focus:ring-0 focus:ring-red-500 " +
        "bg-gray-200 text-gray-900" // Adding darker text and background for focus state
      );
    }
    return "";
  }
    ...
    <div>
      ...
      <div className="explanation">
        ...
        <p>
          ...
          (maximum weight) between {capacityDefaults.min} and{" "}
          {capacityDefaults.max}. You can enter up to {maxNumOfItems} items,
          with values between {itemValueDefaults.min} and{" "}
          {itemValueDefaults.max} and weight between {capacityDefaults.min} and{" "}
          {capacityDefaults.max}. Click the '{submitBtnLabel}' button to step
      ...
      <div>
        <form
          ...
          className="w-full my-4 flex flex-col justify-center gap-4"
        >
          <div className="bg-gray-100 px-2 py-4 sm:px-6 sm:py-6 2xl:mx-auto rounded-lg">
            <label className="mb-3 font-bold text-gray-800" htmlFor="capacity">
              Knapsack Capacity
            </label>
            <input
              type="number"
              ...
              className="w-min ml-4 mb-2 text-gray-900 border border-gray-400 rounded"
              {...register("capacity", {
                valueAsNumber: true,
                max: {
            ...
            {/* Start items */}
            <div>
              <label className="label text-gray-800">Available Items</label>
              {controlledFields.map((item, index) => (
                <div
                  key={item.id}
                  className="border-2 m-2 rounded border-gray-200"
                >
                  <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 bg-gray-100 p-4 relative">
                    <div className="sm:col-span-4 col-span-2">
                      <label
                        htmlFor={`itemsArray.${index}.name`}
                        className="label text-gray-800"
                      >
                        Item Name
                      </label>
                      <input
                        ...
                        className={`${getCSS(index, "name")} text-gray-900`}
                        key={item.id}
                        {...register(`itemsArray.${index}.name`, {
                          validate: {
                            ...
                            isNotEmpty: (v) =>
                              !!v.trim() || "Please enter an item name",
                          },
                        })}
                      />
                    </div>
                    <div className="col-span-1">
                      <label
                        htmlFor={`itemsArray.${index}.value`}
                        className="label text-gray-800"
                      >
                        Value
                      </label>
                      <input
                        ...
                        className={`${getCSS(index, "value")} text-gray-900`}
                        key={item.id}
                    ...
                    <div className="col-span-1">
                      <label
                        htmlFor={`itemsArray.${index}.weight`}
                        className="label text-gray-800"
                      >
                        Weight
                      </label>
                      <input
                        ...
                        min={capacityDefaults.min}
                        max={capacityDefaults.max}
                        className={`${getCSS(index, "weight")} text-gray-900`}
                    ...
                    <button
                      className="absolute -top-2 -right-2 bg-red-200 hover:bg-red-300 rounded-full"
                      type="button"
                    ...
                    <div className="col-span-2 sm:col-span-6 text-gray-800">
                      <div>{displayErrorMsg(`itemsArray.${index}.name`)}</div>
                      <div>{displayErrorMsg(`itemsArray.${index}.value`)}</div>
                      <div>{displayErrorMsg(`itemsArray.${index}.weight`)}</div>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="ml-2 mt-4">
                <button
                  type="button"
                  className="max-w-max btnGreen whitespace-nowrap"
                  disabled={shouldDisableButton()}
                  aria-label={ariaLabelAddNewItem}
                  onClick={() => addItem()}
                >
                  New Item
                </button>
        ...
        <div className="flex flex-col">
          <input
            className="btnBlue w-40 tracking-wide place-self-center"
            type="submit"
            form={formName}
            value={submitBtnLabel}
            aria-label="Start Algorithm Calculation"
      ...
      <div className="explanation">
        ...
        <div>
          <span className="inline-flex items-center">
            where
            <img
              className="h-5 inline pl-1"
              src={eq3}
              alt="where x is either 0 or 1"
              style={{
                filter:
                  "invert(51%) sepia(99%) saturate(401%) hue-rotate(0deg) brightness(95%) contrast(100%)",
              }}
            />
          </span>
        </div>
        <div>
          <span className="inline-flex items-center">
            maximize
            <img
              className="h-12 inline pl-1"
              src={eq1}
              alt="summation equation"
              style={{
                filter:
                  "invert(51%) sepia(99%) saturate(401%) hue-rotate(0deg) brightness(95%) contrast(100%)",
              }}
        ...
        <p>
          ...
          is the item's weight and <em>c</em> is the capacity of the knapsack.{" "}
        </p>
        <p>How does the hiker choose which items to pack?</p>
        <p>
          The brute force solution is to consider every possible combination of
          items. However, this would take O(2<sup>N</sup>) time. Why? The list
          of items is a set.{" "}
          <a className="link" href="https://en.wikipedia.org/wiki/Power_set">
            The number of subsets a set has is 2<sup>N</sup>
          </a>
          . For example, given a set with 3 elements there are 8 subsets, 2
          <sup>3</sup> = 8. For the set [A, B, C] the possible subsets are:
        </p>
        <ol className="list-decimal ml-8">
          <li>[]</li>
          <li>[A]</li>
        ...
        <p>
          ...
          is used to solve the problem. Dynamic programming breaks the problem
        ...
        <p>
          ...
          weights and number of items. Those values are used to build up a table

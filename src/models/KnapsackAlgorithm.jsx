...
class KnapSackAlgorithm {
  ...
  _knapsack() {
    ...
    this.items.forEach((item, index) => {
      ...
      for (
        ...
        const valueForCapacityInPreviousRow =
          table[previousRow][currentCapacity];
        let cellValue = 0;
        if (item.weight <= currentCapacity) {
          let combinedValue = parseFloat(
            format(item.value + table[previousRow][leftOverCapacity])
          );
        ...
        this._maxLengthItem =
          this._maxLengthItem.toString().length <
          table[offsetIndex][currentCapacity].toString().length
            ? table[offsetIndex][currentCapacity]
            : this._maxLengthItem;
      }
    });
    return table;
  ...
  _findItemsThatFit() {
    ...
    for (let i = this.items.length; i > 0; i--) {
      ...
      if (
        this._solutionTable[i][currentCapacity] !==
        this._solutionTable[i - 1][currentCapacity]
      ) {
